...
struct VulkanRuntime::Impl {
 ...
 bool budgetAvailable=false,validationEnabled=false;float budgetFraction=0.8f;double seconds=0,wallSeconds=0,maxSubmissionSeconds=0;uint64_t uploadBytes=0,downloadBytes=0,totalEpochs=0,computeSubmissions=0;uint stateDispatchLimit=65536;
 ...
 void setup(const Snapshot& initial,const std::string& wanted,bool allowSoftware,float requestedBudgetFraction){validate_snapshot(initial);cfg=initial.cfg;if(!(requestedBudgetFraction>0.0f&&requestedBudgetFraction<=0.98f))throw std::runtime_error("Budget fraction must be >0 and <=0.98");budgetFraction=requestedBudgetFraction;
  ...
  if((uint64_t(cfg.opCount)+63)/64>props.limits.maxComputeWorkGroupCount[0])throw std::runtime_error("Operator mutation dispatch exceeds device workgroup limit");
  stateDispatchLimit=uint(std::min<uint64_t>(65536,uint64_t(props.limits.maxComputeWorkGroupCount[0])*64));
  if(props.limits.maxComputeWorkGroupInvocations<64||props.limits.maxComputeWorkGroupSize[0]<64||props.limits.maxComputeSharedMemorySize<31*sizeof(Operator)||props.limits.maxPushConstantsSize<sizeof(Config))throw std::runtime_error("Device cannot hold the 64-lane workgroup, 31-operator shared LUT, or Config push constants");
 ...
 void mutate(){Config dispatch=cfg;dispatch.reserved0=0;vkCmdPushConstants(command,pipelineLayout,VK_SHADER_STAGE_COMPUTE_BIT,0,sizeof(Config),&dispatch);vkCmdBindPipeline(command,VK_PIPELINE_BIND_POINT_COMPUTE,mutatePipeline);vkCmdDispatch(command,uint((uint64_t(cfg.opCount)+63u)/64u),1,1);compute_dependency();}
 void evolve(uint base,uint count){
  Config dispatch=cfg;dispatch.reserved0=base;uint groups=uint((uint64_t(count)+63u)/64u);
 ...
 void advance(uint steps){
  ...
  if(cfg.count<=stateDispatchLimit){
   ...
   for(uint done=0;done<steps;){uint batch=std::min(8u,steps-done);begin_compute();
...
std::string VulkanRuntime::device_json()const{
 ...
 o<<"{\"name\":"<<json_escape(p->props.deviceName)<<",\"vendor_id\":"<<p->props.vendorID<<",\"device_id\":"<<p->props.deviceID<<",\"api_version\":"<<p->props.apiVersion<<",\"driver_version\":"<<p->props.driverVersion
  ...
  <<",\"state_dispatch_limit\":"<<p->stateDispatchLimit<<",\"compute_submission_count\":"<<p->computeSubmissions<<",\"max_submission_seconds\":"<<p->maxSubmissionSeconds
