...
struct VulkanRuntime::Impl {
 ...
 Buffer states[2],overflowStates[2],staging,evolutionWork;Image operators[2];VkSampler lutSampler=VK_NULL_HANDLE;Config cfg{};uint parity=0,queueFamily=0,timestampBits=0,lutWidth=0,lutHeight=0,allocationCount=0;bool splitEvolution=false;
 ...
 bool budgetAvailable=false,validationEnabled=false,synchronizationValidation=false;float budgetFraction=0.8f;double seconds=0,wallSeconds=0,maxSubmissionSeconds=0;uint64_t uploadBytes=0,downloadBytes=0,totalEpochs=0,computeSubmissions=0;uint stateDispatchLimit=65536,workgroupSize=64,statePageCapacity=0,statePaging=0;bool streamedInitialization=false;
 ~Impl(){if(device){vkDeviceWaitIdle(device);if(queries)vkDestroyQueryPool(device,queries,nullptr);if(mutatePipeline)vkDestroyPipeline(device,mutatePipeline,nullptr);if(evolvePipeline)vkDestroyPipeline(device,evolvePipeline,nullptr);for(auto pipeline:splitPipelines)if(pipeline)vkDestroyPipeline(device,pipeline,nullptr);if(descriptorPool)vkDestroyDescriptorPool(device,descriptorPool,nullptr);if(pipelineLayout)vkDestroyPipelineLayout(device,pipelineLayout,nullptr);if(descriptorLayout)vkDestroyDescriptorSetLayout(device,descriptorLayout,nullptr);if(lutSampler)vkDestroySampler(device,lutSampler,nullptr);if(fence)vkDestroyFence(device,fence,nullptr);if(commandPool)vkDestroyCommandPool(device,commandPool,nullptr);for(auto& b:states)destroy(b);for(auto& b:overflowStates)destroy(b);for(auto& b:operators)destroy(b);destroy(staging);destroy(evolutionWork);vkDestroyDevice(device,nullptr);}if(instance){if(messenger){auto destroyMessenger=reinterpret_cast<PFN_vkDestroyDebugUtilsMessengerEXT>(vkGetInstanceProcAddr(instance,"vkDestroyDebugUtilsMessengerEXT"));if(destroyMessenger)destroyMessenger(instance,messenger,nullptr);}vkDestroyInstance(instance,nullptr);}}
 ...
 void upload(Buffer& target,const void* data,size_t bytes,VkDeviceSize base=0){for(size_t offset=0;offset<bytes;){size_t chunk=size_t(std::min<VkDeviceSize>(bytes-offset,staging.size));stage_upload(static_cast<const char*>(data)+offset,chunk,chunk);begin();VkBufferCopy copy{0,base+offset,chunk};vkCmdCopyBuffer(command,staging.handle,target.handle,1,&copy);barrier(VK_PIPELINE_STAGE_TRANSFER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_ACCESS_TRANSFER_WRITE_BIT,VK_ACCESS_SHADER_READ_BIT);finish();uploadBytes+=chunk;offset+=chunk;}}
 void read(Buffer& source,void* data,size_t bytes,VkDeviceSize base=0){for(size_t offset=0;offset<bytes;){size_t chunk=size_t(std::min<VkDeviceSize>(bytes-offset,staging.size));begin();barrier(VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT|VK_PIPELINE_STAGE_TRANSFER_BIT,VK_PIPELINE_STAGE_TRANSFER_BIT,VK_ACCESS_SHADER_WRITE_BIT|VK_ACCESS_TRANSFER_WRITE_BIT,VK_ACCESS_TRANSFER_READ_BIT);VkBufferCopy copy{base+offset,0,chunk};vkCmdCopyBuffer(command,source.handle,staging.handle,1,&copy);barrier(VK_PIPELINE_STAGE_TRANSFER_BIT,VK_PIPELINE_STAGE_HOST_BIT,VK_ACCESS_TRANSFER_WRITE_BIT,VK_ACCESS_HOST_READ_BIT);finish();stage_read(static_cast<char*>(data)+offset,chunk);downloadBytes+=chunk;offset+=chunk;}}
 void upload_states(const State* data,uint first,uint count){
  for(uint done=0;done<count;){uint index=first+done;bool tail=index>=statePageCapacity;uint local=tail?index-statePageCapacity:index;uint length=std::min(count-done,statePageCapacity-local);upload(tail?overflowStates[0]:states[0],data+done,size_t(length)*sizeof(State),VkDeviceSize(local)*sizeof(State));done+=length;}
 }
 void read_states(State* data,uint first,uint count){
  for(uint done=0;done<count;){uint index=first+done;bool tail=index>=statePageCapacity;uint local=tail?index-statePageCapacity:index;uint length=std::min(count-done,statePageCapacity-local);read(tail?overflowStates[parity]:states[parity],data+done,size_t(length)*sizeof(State),VkDeviceSize(local)*sizeof(State));done+=length;}
 }
 ...
 VkPipeline pipeline(const uint32_t* words,size_t bytes,const char* passName){VkShaderModule module=VK_NULL_HANDLE;VkShaderModuleCreateInfo mi{VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO};mi.codeSize=bytes;mi.pCode=words;check(vkCreateShaderModule(device,&mi,nullptr,&module),"vkCreateShaderModule");VkComputePipelineCreateInfo pi{VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO};pi.layout=pipelineLayout;pi.stage.sType=VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;pi.stage.stage=VK_SHADER_STAGE_COMPUTE_BIT;pi.stage.module=module;pi.stage.pName="main";uint values[3]={workgroupSize,statePageCapacity,statePaging};VkSpecializationMapEntry entries[3]={{0,0,sizeof(uint)},{1,sizeof(uint),sizeof(uint)},{2,2*sizeof(uint),sizeof(uint)}};VkSpecializationInfo specialization{3,entries,sizeof(values),values};pi.stage.pSpecializationInfo=&specialization;VkPipeline result=VK_NULL_HANDLE;auto status=vkCreateComputePipelines(device,VK_NULL_HANDLE,1,&pi,nullptr,&result);vkDestroyShaderModule(device,module,nullptr);std::string context=std::string(passName)+" on "+props.deviceName;check(status,context.c_str());return result;}
 void setup(const Snapshot& initial,const std::string& wanted,bool allowSoftware,float requestedBudgetFraction,bool streamInitialization){validate_snapshot(initial,!streamInitialization);streamedInitialization=streamInitialization;cfg=initial.cfg;if(!(requestedBudgetFraction>0.0f&&requestedBudgetFraction<=0.98f))throw std::runtime_error("Budget fraction must be >0 and <=0.98");budgetFraction=requestedBudgetFraction;
  VkApplicationInfo app{VK_STRUCTURE_TYPE_APPLICATION_INFO};app.pApplicationName="Dawnwood Interactive DWI-N1";app.applicationVersion=VK_MAKE_VERSION(0,5,1);app.apiVersion=VK_API_VERSION_1_1;
  ...
  const char* instanceExtensions[2]={VK_EXT_DEBUG_UTILS_EXTENSION_NAME,VK_EXT_VALIDATION_FEATURES_EXTENSION_NAME};VkValidationFeatureEnableEXT syncFeature=VK_VALIDATION_FEATURE_ENABLE_SYNCHRONIZATION_VALIDATION_EXT;VkValidationFeaturesEXT validationFeatures{VK_STRUCTURE_TYPE_VALIDATION_FEATURES_EXT};
  if(validationEnabled){debugInfo.messageSeverity=VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT|VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;debugInfo.messageType=VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT|VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT|VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT;debugInfo.pfnUserCallback=validation_message;debugInfo.pUserData=this;ci.enabledExtensionCount=1;ci.ppEnabledExtensionNames=instanceExtensions;ci.pNext=&debugInfo;const char* syncRequest=std::getenv("VK_LAYER_VALIDATE_SYNC");synchronizationValidation=syncRequest&&std::string(syncRequest)=="1";if(synchronizationValidation){validationFeatures.enabledValidationFeatureCount=1;validationFeatures.pEnabledValidationFeatures=&syncFeature;validationFeatures.pNext=&debugInfo;ci.pNext=&validationFeatures;ci.enabledExtensionCount=2;}}
  ...
  statePageCapacity=tuning_value("DAWNWOOD_STATE_PAGE_RECORDS",props.limits.maxStorageBufferRange/sizeof(State));
  if(statePageCapacity>props.limits.maxStorageBufferRange/sizeof(State)||uint64_t(cfg.count)>2ull*statePageCapacity)throw std::runtime_error("Population exceeds the two-page storage-buffer limit");
  statePaging=cfg.count>statePageCapacity?1u:0u;
  bool rtx5070TiLaptop=props.vendorID==0x10deu&&props.deviceID==0x2f18u;
  workgroupSize=tuning_value("DAWNWOOD_WORKGROUP_SIZE",rtx5070TiLaptop?32u:64u);if(workgroupSize!=32&&workgroupSize!=64&&workgroupSize!=128&&workgroupSize!=256)throw std::runtime_error("Workgroup size must be 32, 64, 128 or 256");
  ...
  stateDispatchLimit=tuning_value("DAWNWOOD_DISPATCH_STATES",rtx5070TiLaptop?1048576u:65536u);
  ...
  if(props.limits.maxPerStageDescriptorStorageBuffers<4||props.limits.maxPerStageDescriptorStorageImages<1||props.limits.maxPerStageDescriptorSampledImages<2||props.limits.maxPerStageDescriptorSamplers<2||props.limits.maxPerStageResources<7)throw std::runtime_error("Device descriptor limits cannot bind the state buffers and sampled/storage operator LUT images");
  ...
  if(splitEvolution&&(props.limits.maxPerStageDescriptorStorageBuffers<5||props.limits.maxPerStageResources<8))throw std::runtime_error("Staged paged evolution requires five storage buffers");
  uint bindingCount=splitEvolution?8u:7u;
  VkDescriptorType descriptorTypes[8]={VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,VK_DESCRIPTOR_TYPE_STORAGE_BUFFER};uint bindingNumbers[8]={0,1,2,3,4,5,6,7};if(!splitEvolution){bindingNumbers[5]=6;bindingNumbers[6]=7;}VkDescriptorSetLayoutBinding bindings[8]{};for(uint k=0;k<bindingCount;++k){uint i=bindingNumbers[k];bindings[k].binding=i;bindings[k].descriptorCount=1;bindings[k].descriptorType=descriptorTypes[i];bindings[k].stageFlags=VK_SHADER_STAGE_COMPUTE_BIT;}
  ...
  VkDeviceSize primaryBytes=VkDeviceSize(std::min(cfg.count,statePageCapacity))*sizeof(State);
  for(auto& b:states)make_buffer(b,primaryBytes,VK_BUFFER_USAGE_STORAGE_BUFFER_BIT|VK_BUFFER_USAGE_TRANSFER_SRC_BIT|VK_BUFFER_USAGE_TRANSFER_DST_BIT,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
  if(statePaging)for(auto& b:overflowStates)make_buffer(b,sb-primaryBytes,VK_BUFFER_USAGE_STORAGE_BUFFER_BIT|VK_BUFFER_USAGE_TRANSFER_SRC_BIT|VK_BUFFER_USAGE_TRANSFER_DST_BIT,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
  ...
  VkDescriptorPoolSize ps[3]={{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,splitEvolution?10u:8u},{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,4},{VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,2}};VkDescriptorPoolCreateInfo dpi{VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO};dpi.maxSets=2;dpi.poolSizeCount=3;dpi.pPoolSizes=ps;check(vkCreateDescriptorPool(device,&dpi,nullptr,&descriptorPool),"vkCreateDescriptorPool");VkDescriptorSetLayout layouts[2]={descriptorLayout,descriptorLayout};VkDescriptorSetAllocateInfo da{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO};da.descriptorPool=descriptorPool;da.descriptorSetCount=2;da.pSetLayouts=layouts;check(vkAllocateDescriptorSets(device,&da,sets),"vkAllocateDescriptorSets");
  for(uint p=0;p<2;++p){auto& oldTail=statePaging?overflowStates[p]:states[p];auto& newTail=statePaging?overflowStates[1-p]:states[1-p];VkDescriptorBufferInfo bi[5]={{states[p].handle,0,primaryBytes},{states[1-p].handle,0,primaryBytes},{evolutionWork.handle,0,evolutionWork.size},{oldTail.handle,0,oldTail.size},{newTail.handle,0,newTail.size}};VkDescriptorImageInfo ii[3]={{lutSampler,operators[p].view,VK_IMAGE_LAYOUT_GENERAL},{VK_NULL_HANDLE,operators[1-p].view,VK_IMAGE_LAYOUT_GENERAL},{lutSampler,operators[1-p].view,VK_IMAGE_LAYOUT_GENERAL}};VkWriteDescriptorSet writes[8]{};for(uint k=0;k<bindingCount;++k){uint i=bindingNumbers[k];auto& write=writes[k];write.sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;write.dstSet=sets[p];write.dstBinding=i;write.descriptorCount=1;write.descriptorType=descriptorTypes[i];if(i<2)write.pBufferInfo=&bi[i];else if(i>=5)write.pBufferInfo=&bi[i-3];else write.pImageInfo=&ii[i-2];}vkUpdateDescriptorSets(device,bindingCount,writes,0,nullptr);}
  ...
  if(streamInitialization){std::vector<State> chunk(std::min(cfg.count,65536u));for(uint64_t base=0;base<cfg.count;base+=chunk.size()){uint count=uint(std::min<uint64_t>(chunk.size(),uint64_t(cfg.count)-base));for(uint i=0;i<count;++i)chunk[i]=initialize_state(cfg,uint(base)+i);upload_states(chunk.data(),uint(base),count);}}else upload_states(initial.states.data(),0,cfg.count);upload(operators[0],initial.ops.data(),size_t(ob));
 }
...
VulkanRuntime::VulkanRuntime(const Snapshot& s,const std::string& name,bool allowSoftware,float budgetFraction,bool streamInitialization):p(new Impl){p->setup(s,name,allowSoftware,budgetFraction,streamInitialization);}
VulkanRuntime::~VulkanRuntime()=default;
void VulkanRuntime::advance(uint steps){p->advance(steps);}
Snapshot VulkanRuntime::download_metadata(){Snapshot s;s.cfg=p->cfg;s.ops.resize(s.cfg.opCount);p->read(p->operators[p->parity],s.ops.data(),s.ops.size()*sizeof(Operator));return s;}
Snapshot VulkanRuntime::download(){Snapshot s=download_metadata();s.states.resize(s.cfg.count);p->read_states(s.states.data(),0,s.cfg.count);return s;}
void VulkanRuntime::visit_states(const std::function<void(const State*,uint,uint)>& visitor){std::vector<State> chunk(std::min(p->cfg.count,65536u));for(uint64_t base=0;base<p->cfg.count;base+=chunk.size()){uint count=uint(std::min<uint64_t>(chunk.size(),uint64_t(p->cfg.count)-base));p->read_states(chunk.data(),uint(base),count);visitor(chunk.data(),uint(base),count);}}
...
std::string VulkanRuntime::device_json()const{
 ...
 uint64_t buffers=p->staging.allocated+p->evolutionWork.allocated,images=0;for(auto& b:p->states)buffers+=b.allocated;for(auto& b:p->overflowStates)buffers+=b.allocated;for(auto& b:p->operators)images+=b.allocated;
 std::ostringstream o;
 o<<"{\"name\":"<<json_escape(p->props.deviceName)<<",\"vendor_id\":"<<p->props.vendorID<<",\"device_id\":"<<p->props.deviceID<<",\"api_version\":"<<p->props.apiVersion<<",\"driver_version\":"<<p->props.driverVersion
  <<",\"software_device\":"<<(software()?"true":"false")<<",\"validation_layer_enabled\":"<<(p->validationEnabled?"true":"false")<<",\"synchronization_validation_enabled\":"<<(p->synchronizationValidation?"true":"false")<<",\"validation_errors\":"<<p->validationErrors.load()<<",\"validation_warnings\":"<<p->validationWarnings.load()
  ...
  <<",\"streamed_initialization\":"<<(p->streamedInitialization?"true":"false")<<",\"host_state_chunk_bytes\":"<<(p->streamedInitialization?uint64_t(std::min(p->cfg.count,65536u))*sizeof(State):0)
  <<",\"state_page_capacity\":"<<p->statePageCapacity<<",\"state_pages_per_buffer\":"<<(p->statePaging?2:1)<<",\"max_state_records\":"<<std::min<uint64_t>(UINT32_MAX,2ull*p->statePageCapacity)
