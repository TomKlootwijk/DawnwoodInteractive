// DWI-RESIDENT-0.3 resident definition-selection and epoch transaction runtime.
// Existing N1/D1 State, Operator, Config and checkpoint ABIs are unchanged.
#include "resident_v3.inc"
...
constexpr char residentMagic[8]={'D','W','R','D','0','0','0','3'};
constexpr uint exactLimit=16777215u,targetSelector=UINT32_MAX,maxStates=128u;
struct Function {uint code,outputs,instructions,inputs,width,signature;};
struct Family {uint interfaceId;std::map<uint,uint> methods;};
struct Program {
 std::vector<uint> config,images;
 std::vector<Function> functions;
 std::vector<Family> families;
 std::map<uint,std::pair<uint,uint>> signatures;
 std::map<uint,std::map<uint,uint>> interfaces;
...
void validate_function(const Program& p,const Function& f,uint ordinal){
 ...
 for(uint i=0;i<f.instructions;++i){
  ...
  const std::string at=where+" instruction "+std::to_string(i);ensure(op<=19,at+": unknown XIR opcode");
  if(op==0){ensure(finite(dw_bits_float(a)),at+": nonfinite constant");ensure(!b&&!c,at+": unused operands must be zero");}
   ...
   ensure(a<i,at+": operand a is not a back-reference");bool binary=op==2||op==3||op==4||op==5||op==11||op==12||op==15||op==19;
...
void validate_plan(const Program& p,bool mutation){
 ...
 std::array<bool,256> initialized{};std::array<bool,maxStates> written{};
 ...
 for(uint step=0;step<steps;++step){
  ...
  ensure(op<=12,at+": unknown opcode");uint used=0;
  ...
  if(op==0){used=3;ensure(finite(dw_bits_float(w[2])),at+": nonfinite constant");write(w[1],1,at);}
  ...
  else if(op==2||op==12){used=6;if(op==2)selector(w[2]);else read(w[2],1,at);range(w[3],w[4],24,at);ensure(w[5]<=1&&(!mutation||w[5]==0),at+": forbidden record read bank");write(w[1],w[4],at);}
   ...
   used=8;if(op==4)selector(w[1]);else read(w[1],1,at);ensure(w[2]<3,at+": invalid record slot");ensure(w[3]==(mutation?0u:1u),at+": forbidden record call bank");
   auto signature=p.signatures.find(w[6]);ensure(signature!=p.signatures.end(),at+": unknown call signature");
   ensure(w[2]==0||w[7]==0,at+": non-body role must be zero");
   auto compatible=[&](uint record){uint declared=p.config[8u+4u*record+1u+w[2]];if(w[2]==0){const auto& roles=p.interfaces.at(declared);auto role=roles.find(w[7]);return role!=roles.end()&&role->second==w[6];}return declared==w[6];};
   if(op==4){if(w[1]==targetSelector){for(uint record=0;record<p.records;++record)ensure(compatible(record),at+": target slot has incompatible interface role/signature");}else ensure(compatible(w[1]),at+": selected slot has incompatible interface role/signature");}
   call(signature->second.first,signature->second.second,w[4],w[5]);
  }
  else if(op==5||op==6){
  ...
  else if(op==10){used=4;read(w[2],w[3],at);write(w[1],w[3],at);}
...
bool possible_xir_failure(const Program& p,const uint* image,const uint* step,uint phase,uint target,uint detail){
 uint ordinal=detail>>16u,reason=detail&65535u;if(!ordinal||reason<1||reason>6)return false;
 auto possible=[&](const Function& f){if(ordinal>f.instructions)return false;uint opcode=p.config[p.heapOffset+f.code+4u*(ordinal-1u)];return reason==3||(reason==1&&opcode==5)||(reason==2&&opcode==9)||(reason==4&&opcode==17)||(reason==5&&opcode==18)||(reason==6&&opcode==19);};
 if(step[0]==3)return possible(p.functions[step[1]]);
 if(step[0]!=4&&step[0]!=11)return false;
 if(phase==1){
  auto reachable=[&](uint record){uint handle=image[6u+p.stateWidth+24u*record+step[2]];if(step[2]==0){if(handle>=p.families.size())return false;auto method=p.families[handle].methods.find(step[7]);if(method==p.families[handle].methods.end())return false;handle=method->second;}return handle<p.functions.size()&&p.functions[handle].signature==step[6]&&possible(p.functions[handle]);};
  if(step[0]==4)return reachable(step[1]==targetSelector?target:step[1]);
  for(uint record=0;record<p.records;++record)if(reachable(record))return true;return false;
 }
...
void validate_failure(const Program& p,const uint* image,const std::string& where){
 ...
 ensure(epoch<=exactLimit,where+": epoch exceeds exact integer bound");ensure(status<=11,where+": unknown failure status");
 ...
 else if(status==2)ensure((post&&detail<3u)||((w[0]==4u||w[0]==11u)&&detail>=(w[2]==0?p.families.size():p.functions.size())),where+": inconsistent invalid-handle detail");
 ...
 else if(status==4)ensure((post&&detail<3)||((w[0]==4||w[0]==11)&&detail==w[2]),where+": inconsistent signature failure");
 ...
 else if(status==9)ensure((w[0]==11&&detail==w[1])||(w[0]==12&&detail==w[2]),where+": inconsistent computed-source failure");
 else if(status==10){bool found=false;for(uint r=0;r<p.records;++r)found|=p.config[8u+4u*r]==detail;ensure((w[0]==11||w[0]==12)&&detail<=exactLimit&&!found,where+": inconsistent absent-source failure");}
 else if(status==11)ensure((w[0]==4||w[0]==11)&&detail==w[6],where+": inconsistent typed-role failure");
}
void validate_images(const Program& p,const std::vector<uint>& images){
 ...
 for(uint lane=0;lane<p.count;++lane){
  ...
  for(uint record=0;record<p.records;++record){const uint* words=image+6u+p.stateWidth+24u*record;
   for(uint slot=0;slot<3;++slot){ensure(words[slot]<(slot==0?p.families.size():p.functions.size()),where+": unresolved record handle");uint declared=slot==0?p.families[words[slot]].interfaceId:p.functions[words[slot]].signature;ensure(declared==p.config[8u+4u*record+1u+slot],where+": record handle interface/signature mismatch");}
...
Program read_program(const std::filesystem::path& path){
 ...
 std::ifstream in(path,std::ios::binary|std::ios::ate);if(!in)throw std::runtime_error("Cannot open input: "+path.string());auto end=in.tellg();ensure(end>=std::streamoff(40),"Input shorter than DWRD0003 header");uint64_t bytes=uint64_t(end);in.seekg(0);
 char magic[8];uint h[8];read_exact(in,magic,8);read_exact(in,h,sizeof(h));ensure(!std::memcmp(magic,residentMagic,8),"Expected DWRD0003 magic");
 Program p;p.count=h[0];p.records=h[1];p.stateWidth=h[2];p.heapWords=h[4];p.mutationSteps=h[5];p.actionSteps=h[6];
 ensure(p.count>0,"count must be positive");ensure(p.records>=1&&p.records<=32,"recordCount must be 1..32");ensure(p.stateWidth>=1&&p.stateWidth<=maxStates,"stateWidth must be 1..128");ensure(h[3]>=1&&h[3]<=256,"functionCount must be 1..256");ensure(p.mutationSteps>=1&&p.mutationSteps<=4096&&p.actionSteps>=1&&p.actionSteps<=4096,"Plan lengths must be 1..4096");ensure(h[7]>=1&&h[7]<=256,"familyCount must be 1..256");
 uint64_t heap=8u+4u*p.records+6u*h[3]+4u*h[7],mutation=heap+p.heapWords,action=mutation+8u*p.mutationSteps,configWords=action+8u*p.actionSteps;
 ...
 for(uint i=0;i<h[7];++i){
  const uint* f=p.config.data()+8u+4u*p.records+6u*h[3]+4u*i;std::string where="Family "+std::to_string(i);
  ensure(f[0]!=0,where+": interface must be positive");ensure(f[2]>=1&&f[2]<=32,where+": method count must be 1..32");ensure(f[3]==0,where+": reserved word must be zero");ensure(uint64_t(f[1])+2u*f[2]<=p.heapWords,where+": methods outside heap");
  Family family{f[0],{}};std::map<uint,uint> contract;uint previous=0;
  for(uint j=0;j<f[2];++j){const uint* method=p.config.data()+p.heapOffset+f[1]+2u*j;ensure(j==0||method[0]>previous,where+": method roles must be strictly increasing");previous=method[0];ensure(method[1]<p.functions.size(),where+": unresolved method function");family.methods.emplace(method[0],method[1]);contract.emplace(method[0],p.functions[method[1]].signature);}
  auto entry=p.interfaces.emplace(family.interfaceId,contract);ensure(entry.second||entry.first->second==contract,where+": families sharing an interface have different role/signature mappings");p.families.push_back(std::move(family));
 }
 std::set<uint> identities;for(uint i=0;i<p.records;++i){const uint* meta=p.config.data()+8u+4u*i;ensure(identities.insert(meta[0]).second,"Duplicate source index");ensure(meta[0]<=exactLimit,"Source index must be within 0..16777215");ensure(meta[1]!=0&&p.interfaces.count(meta[1]),"Immutable body interface has no matching family");for(uint slot=2;slot<4;++slot)ensure(meta[slot]!=0&&p.signatures.count(meta[slot]),"Immutable record signature has no matching bank function");}
...
struct Vulkan {
 ...
 void initialize(const std::string& wanted){
  ...
  VkApplicationInfo app{};app.sType=VK_STRUCTURE_TYPE_APPLICATION_INFO;app.pApplicationName="Dawnwood DWI-RESIDENT-0.3";app.applicationVersion=VK_MAKE_VERSION(0,2,0);app.apiVersion=VK_API_VERSION_1_1;
...
int main(int argc,char** argv){
 ...
 try{
  ensure(argc>=2&&std::string(argv[1])=="run","Usage: dawnwood-resident-v3 run --input file --output file --backend cpu|vulkan --epochs N [--device substring] [--reverse-targets]");
  ...
  Program p=read_program(input);Result r;if(backend=="cpu")r=run_cpu(p,epochs,reverse);else {Vulkan runtime;runtime.initialize(wanted);r=runtime.run(p,executable_directory(argv[0])/"resident_v3.spv",epochs,reverse);}
  write_result(output,p,r);uint64_t failures=0,previousFailures=0,commits=0;uint minimumEpoch=UINT32_MAX,maximumEpoch=0;std::array<uint64_t,12> statuses{};
