// DWI-RESIDENT-0.2 resident definition-selection and epoch transaction runtime.
...
constexpr char residentMagic[8]={'D','W','R','D','0','0','0','2'};
...
Program read_program(const std::filesystem::path& path){
 ...
 std::ifstream in(path,std::ios::binary|std::ios::ate);if(!in)throw std::runtime_error("Cannot open input: "+path.string());auto end=in.tellg();ensure(end>=std::streamoff(40),"Input shorter than DWRD0002 header");uint64_t bytes=uint64_t(end);in.seekg(0);
 char magic[8];uint h[8];read_exact(in,magic,8);read_exact(in,h,sizeof(h));ensure(!std::memcmp(magic,residentMagic,8),"Expected DWRD0002 magic");
...
struct Vulkan {
 ...
 void initialize(const std::string& wanted){
  ...
  VkApplicationInfo app{};app.sType=VK_STRUCTURE_TYPE_APPLICATION_INFO;app.pApplicationName="Dawnwood DWI-RESIDENT-0.2";app.applicationVersion=VK_MAKE_VERSION(0,2,0);app.apiVersion=VK_API_VERSION_1_1;
...
int main(int argc,char** argv){
 ...
 try{
  ensure(argc>=2&&std::string(argv[1])=="run","Usage: dawnwood-resident-v2 run --input file --output file --backend cpu|vulkan --epochs N [--device substring] [--reverse-targets]");
  ...
  Program p=read_program(input);Result r;if(backend=="cpu")r=run_cpu(p,epochs,reverse);else {Vulkan runtime;runtime.initialize(wanted);r=runtime.run(p,executable_directory(argv[0])/"resident_v2.spv",epochs,reverse);}
