#!/usr/bin/env python3
"""Build DWI-RESIDENT-0.3 separately, preserving N1/D1, resident-v2 and XIR artifacts."""
...
def main() -> int:
    ...
    parser.add_argument('--output', type=Path, default=ROOT / 'bin/windows-resident-v3')
    ...
    shader, binary = output / 'resident_v3.spv', output / 'dawnwood-resident-v3.exe'
    sources = {name: ROOT / name for name in (
        'include/resident_v3.inc', 'include/source_math_v2.inc', 'include/numeric_types.inc', 'include/numeric_math.inc',
        'src/resident_v3_cli.cpp', 'shaders/resident_v3.comp', 'tools/build_resident_v3.py')}
    sources['repo:docs/RESIDENT_V3_ABI.md'] = REPO / 'docs/RESIDENT_V3_ABI.md'
    initial_hashes = {name: digest(path) for name, path in sources.items()}
    commands = [
        [str(glslc), '--target-env=vulkan1.1', f'-I{ROOT / "include"}',
         str(ROOT / 'shaders/resident_v3.comp'), '-o', str(shader)],
        ...
        [str(compiler), '-std=c++17', '-O2', '-Wall', '-Wextra', '-Wpedantic',
         ...
         str(ROOT / 'src/resident_v3_cli.cpp'), str(vulkan), '-o', str(binary)],
    ]
    receipts = []
    for i, command in enumerate(commands):
        ...
        if completed.returncode:
            (output / 'build.json').write_text(json.dumps({
                'profile': 'DWI-RESIDENT-0.3', 'completed': False,
    ...
    if initial_hashes != final_hashes:
        failure = {'profile': 'DWI-RESIDENT-0.3', 'completed': False,
    ...
    report = {'profile': 'DWI-RESIDENT-0.3', 'completed': True,
