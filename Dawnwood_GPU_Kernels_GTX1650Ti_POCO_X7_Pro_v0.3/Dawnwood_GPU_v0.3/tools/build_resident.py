#!/usr/bin/env python3
"""Build DWI-RESIDENT-0.1 separately, preserving N1/D1 and XIR artifacts."""
...
def main() -> int:
    ...
    parser.add_argument('--output', type=Path, default=ROOT / 'bin/windows-resident')
    ...
    shader, binary = output / 'resident.spv', output / 'dawnwood-resident.exe'
    sources = {name: ROOT / name for name in (
        'include/resident.inc', 'include/numeric_types.inc', 'include/numeric_math.inc',
        'src/resident_cli.cpp', 'shaders/resident.comp', 'tools/build_resident.py')}
    sources['repo:docs/RESIDENT_ABI.md'] = REPO / 'docs/RESIDENT_ABI.md'
    initial_hashes = {name: digest(path) for name, path in sources.items()}
    commands = [
        [str(glslc), '--target-env=vulkan1.1', f'-I{ROOT / "include"}',
         str(ROOT / 'shaders/resident.comp'), '-o', str(shader)],
        ...
        [str(compiler), '-std=c++17', '-O2', '-Wall', '-Wextra', '-Wpedantic',
         ...
         str(ROOT / 'src/resident_cli.cpp'), str(vulkan), '-o', str(binary)],
    ]
    receipts = []
    for i, command in enumerate(commands):
        ...
        if completed.returncode:
            (output / 'build.json').write_text(json.dumps({
                'profile': 'DWI-RESIDENT-0.1', 'completed': False,
    ...
    if initial_hashes != final_hashes:
        failure = {'profile': 'DWI-RESIDENT-0.1', 'completed': False,
    ...
    report = {'profile': 'DWI-RESIDENT-0.1', 'completed': True,
