...
from packing import encode_bc5,decode_bc5,memory_model,memory_model_v05,bc5_bytes
...
def main():
    ...
    def succeeded(record):
        ...
        return record.get('exit_code')==0 and isinstance(report,dict) and 'error' not in report and report.get('passed',True) is not False and report.get('health',{}).get('passed',True) is not False
    gpu_available=succeeded(probe) and bool(probe['report'].get('device'))
    verify=None
    ...
    for i in range(31):
        ...
        effect=(changed['report'].get('state_only_fnv1a64')!=baseline['report'].get('state_only_fnv1a64')) if succeeded(baseline) and succeeded(changed) and baseline['report'].get('state_only_fnv1a64') and changed['report'].get('state_only_fnv1a64') else None
    ...
    same=all(succeeded(r) for r in [baseline,first,second]) and bool(baseline['report'].get('state_and_operator_fnv1a64')) and baseline['report'].get('state_and_operator_fnv1a64')==second['report'].get('state_and_operator_fnv1a64')
    ...
    current_memory=memory_model_v05(4096,max_image_dimension_2d=device.get('max_image_dimension_2d'),split_evolution=device.get('split_evolution',False),state_dispatch_limit=device.get('state_dispatch_limit',65536))
    packing={'kind':'CPU BC5 format/approximation experiment','encoded_block_bytes':len(coded),'two_raw_UNORM8_channels_bytes':32,'rmse_UNORM8_units':error,'arbitrary_input_lossless':error==0,'bc5_4096_base_bytes':bc5_bytes(4096,4096),'working_set_4096':current_memory,'legacy_working_set_4096':memory_model(4096),'raw_budget_examples':{str(b):{'rgba32f_elements':b//16,'rgba8_elements':b//4,'bc5_aligned_average_texels':b} for b in [4*10**9,4*2**30,12*2**30]}}
        ...
        for backend in (['cpu','vulkan'] if gpu_available else ['cpu']):
            for count in [256,1024,4096]:
                ...
                for repeat in range(3):
                    ...
                    if succeeded(rec) and 'seconds' in rec['report']:samples.append(rec['report']['seconds'])
    ...
    comparison_passed=bool(succeeded(verify) and verify['report'].get('passed',False)) if verify else None
    target_executed=bool(device.get('vendor_id')==0x10de and '1650 Ti' in device.get('name','') and not device.get('software_device',True) and verify)
    report={'profile':cpu['report'].get('profile','unreported'),'environment':{'python':sys.version,'platform':platform.platform()},'python_tests':python_summary,'cpu_tests_passed':succeeded(cpu) and cpu['report'].get('passed',False),'vulkan_device':device,'vulkan_comparison_executed':verify is not None,'vulkan_comparison_passed':comparison_passed,'checkpoint_replay_passed':same,'body_coverage_passed':all(x['numerical_state_changed'] is True for x in coverage),'body_coverage':coverage,'packing_rmse':error,'real_GTX_1650_Ti_test':target_executed,'real_GTX_1650_Ti_passed':bool(target_executed and comparison_passed),'real_POCO_test':False,'phone_note':'Only tools/test_phone.py can populate a connected-phone report; no phone result is inferred from shader compilation.'}
    (a.out/'summary.json').write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(report,indent=2))
    passed=tests.wasSuccessful() and succeeded(cpu) and cpu['report'].get('passed',False) and same and all(x['numerical_state_changed'] is True for x in coverage)
    if verify:passed &= bool(comparison_passed)
    if a.device and not gpu_available:passed=False
    if a.bench:passed &= all(succeeded(r) for r in commands if r['name'].startswith('bench_'))
