...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle saved) {
        ...
        TextView title=new TextView(this);title.setText("Dawnwood Interactive\nNumerical Vulkan kernel · DWI-N1 0.3");title.setTextSize(20);layout.addView(title);
        ...
        if(getIntent().getBooleanExtra("autorun",false)){
            ...
            launch(command==null?"verify":command,getIntent().getIntExtra("count",128),getIntent().getIntExtra("steps",16),runId==null?"adb":runId);
        }
    }
    private void addButton(LinearLayout layout,String text,String command,int count,int steps){Button b=new Button(this);b.setText(text);b.setOnClickListener(v->launch(command,count,steps,"interactive_"+System.currentTimeMillis()));buttons.add(b);layout.addView(b);}
    private JSONObject deviceContext() throws Exception {
        ...
        ActivityManager.MemoryInfo info=new ActivityManager.MemoryInfo();((ActivityManager)getSystemService(ACTIVITY_SERVICE)).getMemoryInfo(info);d.put("system_total_ram_bytes",info.totalMem);d.put("system_available_ram_bytes",info.availMem);d.put("system_low_memory",info.lowMemory);
    ...
    private void launch(String command,int count,int steps,String runId){
        if(busy)return;
        ...
        executor.execute(()->{
            ...
            try{
                ...
                String[] args={command,"--count",Integer.toString(count),"--steps",Integer.toString(steps),"--batch","8"};
