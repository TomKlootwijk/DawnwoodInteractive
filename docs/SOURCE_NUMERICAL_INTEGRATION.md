...
**The authored `cycle_v0.1` edition executes the eight-stage core through resident definitions on CPU and the RTX 5070 Ti Laptop GPU. The DWI-ENZYME-0.1 application now adds resident enzyme-pool reconciliation inside that cycle. Both use the bounded DWI-RESIDENT-0.2 runtime; the earlier XIR, Apply and resident-v1 components retain their narrower scopes. Selected-action coverage, optional packing, unrestricted authoring and numerical accuracy limits remain explicit.**
...
The [resident enzyme binding](RESIDENT_ENZYME_BINDING.md) extends that core with
proposal, chemical SDF, projection/evaluation, acceptance and witness roles. Its
35 families, 61 functions and 64 state words preserve the original action prefix
and return chemical history to resident mutation. The [application campaign](../output/resident_enzyme_2026-09-24/REPORT.md)
supplies domain-specific evidence; the earlier cycle's saturation results are not
an enzyme accuracy or speedup measurement.
...
| Implementation | DWI-RESIDENT-0.2 executes typed current definitions and their call tapes; cycle_v0.1 supplies the eight-stage authored application. DWI-ENZYME-0.1 supplies explicit resident domain roles. Earlier XIR/Apply/resident-v1 profiles remain component references. |
...
| Resident mutation and metarule self-action | Verified finite 31-record editions | All records receive old-snapshot mutation/rebinding. The core has three interfaces with alternate families; the enzyme application adds compatible blend strategies. Freezing the mutator's successor changes accepted concentrations in 81/129 cases by epoch 2. Arbitrary expression synthesis remains outside these profiles. |
| Numerical-cycle checkpoint continuation | Verified for recorded resident runs | Full bank, typed families, tapes, live handles, anchors and all state words survive exact continuation: 44 for the core, 64 for the enzyme application. Source snapshots and stage ranges accompany compilation. |
...
A compiled situated record needs a stable identity/index, versioned executable body, field program, anchor program or state, parameters and provenance. The resident editions implement this bounded record contract. Application calls use typed operands and the current record; mutation additionally reads the immutable preceding whole state and LUT. None of these requirements implies that host interpreter instructions or scientific constants must rewrite themselves.
...
The table preserves the required types and source/binding boundaries. The [current cycle contract](SOURCE_CYCLE_BINDING.md) identifies the concrete lowered forms and actual call paths; its core stages are now executable. Optional BC5/Bayer layout, unrestricted expressions and selected-action adapters beyond the declared six-record subset remain outside that edition. N1 helper availability alone is not evidence that a source call has been compiled.
...
| 4 `phyllotaxis` | Phase difference, primitive results, LUT and interval produce divergence/placement information. | Early p7 supplies a planar golden-angle seed policy; cycle_v0.1 declares its chart mapping and evolving numerical action separately. |
...
| 7 `T_shape` | Local point and stem/bar dimensions produce a field value. | Current cycle uses the exact-distance formula for an eight-edge 2D polygon; a minimum of box distances would not have the same interior-distance claim. |
| 8 `pyramid` | Local point and dimensions produce a field value. | Current cycle binds a triangular side in 2D with exact segment-distance/sign formulas, distinct from a 3D pyramid SDF. |
| 9 `circle` | Local 2D point and positive radius produce signed distance. | Existing Euclidean radial expression is reusable. |
| 10 `cone` | Local 3D point and dimensions produce a field value. | Current cycle uses a declared capped-cone distance formula; early residual formulas remain separate choices. |
...
| 14 `blend` | Typed coupling, growth and primitive values produce the core frame; additional enzyme roles produce candidates, distances and acceptance. | Chemical roles are an explicit application extension. The original role 0 and carrier field retain their meanings; chemical SDF is resident body role 101. |
...
The original recurrence does not directly apply resident entries 15, 20 and 29: its raw seed, session controls and PSI symbol supply those roles. The numerical cycle explicitly invokes the current PSI definition; wavefront initialization and jitter remain supplied inputs. Entry 26 remains optional and unsupported for device encoding. Catalogue coverage describes actual call paths rather than counting stored names.
...
## Remaining scope and application accuracy

The bounded eight-stage cycle, old-snapshot full-catalogue mutation, typed paired state and checkpoint continuation are implemented. The enzyme application now expresses candidate operations, chemical-law evaluation, acceptance, returned observations/history and strategy changes through those resident definitions. Authoritative scientific laws remain fixed while the proposal family changes. A stable interpreter executes the definitions; native instruction rewriting is not required for this form of self-reference.

The measured enzyme task is synthetic candidate consistency under a supplied closed-pool mechanism and equal-weight concentration metric. It does not establish kinetics, thermodynamic direction, biochemical time, experimental validity or superior optimization performance. FP32 boundary effects can leave unresolved gaps and can accept tiny real-objective increases; the computed gap is an estimate. Independent reference comparisons, rather than CPU/GPU agreement alone, determine the reported accuracy.

Broader selected-role adapters, optional Bayer/BC5 device layouts, unrestricted source-language compilation, arbitrary resident expression synthesis and application speed advantages remain unestablished. Physical quantum behavior, general intelligence and the source discussion's larger universality claims also remain unproved. The [enzyme report](../output/resident_enzyme_2026-09-24/REPORT.md) and [binding contract](RESIDENT_ENZYME_BINDING.md) retain the concrete useful result and its limits.
