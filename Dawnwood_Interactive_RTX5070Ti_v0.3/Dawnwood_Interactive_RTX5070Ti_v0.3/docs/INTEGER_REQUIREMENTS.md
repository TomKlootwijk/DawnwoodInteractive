...
The computational mechanisms and the scoped acceptance checks below are complete for the delivered integer-1 executables. The evidence index is [final_summary.json](../results/integer/final_summary.json); commands, source/binary identities, raw outcomes and limitations are retained in its linked artifacts. Checkmarks mean implementation plus evidence at the stated tested scope, not a proof over every possible state or physical quantum validation.

- [x] Integer edition has a distinct model identifier and recorded arithmetic ABI; historical floating results are not relabeled as integer results.
- [x] Every component INT-01 through INT-16 participates in the same actual CUDA run; the optional readout remains downstream.
- [x] All recurrent functions accept integer state/configuration and execute integer calculations, including seed initialization, signed supports, K, Hadamard, log/phase encoding, fourth-slot calculus, inverse feedback, mutation and both codecs.
- [x] Compiled kernel call-graph/PTX/SASS inspection records integer versus floating arithmetic, transcendental instructions and conversion/fetch boundaries. The documented native sampler/representation transfers and constant-only HFMA2 materialization are the explicit exceptions; no dynamic-input floating arithmetic is allowed. Float report formatting outside the recurrence is identified separately.
- [x] The small LUT is built from reproducible integer data/functions; its complete allocated and shared sizes are reported. Quarter-wave and complex antipodal symmetries are checked.
- [x] Every nonzero LP8 token roundtrips; all zero aliases decode zero; equal/opposite integer complex pairs cancel at the algebraic stage as defined. Exhaustive pair comparisons quantify encoding changes rather than asserting physical norm conservation.
- [x] Every named support returns a signed integer magnitude, passes interior/boundary/exterior cases and has measured quantization error against an independent reference. K seam, fold parity and negative winding are checked.
- [x] Integer rounding, overflow bounds, signed division conventions and all bit shifts are reviewed. Hardware sanitizer checks exercise partial windows and page boundaries.
- [x] Forced four-stage update is compared stage by stage with an independent integer reference, including the explicit two-unit fourth-stage event. A zero-event control demonstrates the chosen difference.
- [x] All 65,536 phase inverse words compose to zero modulo 65,536; T's scope is stated. Source `[0,2,0,1]` words and decoded values are recorded.
- [x] Full integer CPU/GPU state or snapshots agree where they use the same decoder law; native BC5 palette differences are explicitly scoped. Exact native GPU reference/optimized state comparisons cover both codecs, multiple seeds, partial/full windows, mutation/jitter controls, zero hops and inverse-gain controls. Integer determinism is tested, not inferred from the word integer.
- [x] Restart/continuation and graph/sequential execution preserve the integer state. Historical floating snapshots cannot silently resume under another numerical binding.
- [x] Self-reference is shown by executable-word, support, link and location changes and by later dependence on those changes, with a frozen-control run.
- [x] Exercised large-field allocation with explicit headroom initializes useful payload, completes a full sweep, records actual free/reserved bytes and runs without recurrent host field hot-swapping. OS residency claims remain scoped to actual evidence.
- [x] Repeated optimized/reference timings compare the **same integer law** at fixed W, dt, gain, seed, hops, codec and cache policy when isolating implementation changes. Comparison with the old floating law is labeled a different-model comparison.
- [x] Report contains speed distribution, stored pairs, updated pairs/s, full-sweep time, total bytes, cache/profiler scope, numerical error and source correspondence. Every claimed result points to a fresh integer-edition artifact.
- [x] A new formalization/report describes the implemented integer choices and remaining limitations without overwriting the prior historical report.
...
Integer antipodal cancellation removes the earlier floating cancellation residue at that component. It does not remove log-radius wrapping or establish a unitary full LP8 recurrence. The additional encoded-pair and quantum-mechanism diagnostics below measure those remaining limitations.

### Delivered GPU acceptance and performance evidence

The [final GPU validation](../results/integer/validation_fast_masks/validation.json) passed 46 complete-snapshot equality or rejection checks over 70 processes, including four implementations, both codecs, partial windows, graph/sequential equality, continuation and rejection of legacy version-3 snapshots. CPU stage checks use actual native-decoded BC5 input bytes, so the measured at-most-one-token CPU/hardware palette difference is not hidden by a tolerance on later states. RG8 fetches are exact. Each GPU self-test checks all 3,840 live support predicates, complete 64-byte stages and all 31 mutated operator records against the respective CPU oracle.

The [sign-predicate checks](../results/integer/integer_predicate_tests.txt) passed 187,164 exact comparisons plus 3,598 boundary/coverage assertions, including all named shapes. The [D4 component cache checks](../results/integer/integer_pair_cache_tests.txt) passed 1,253,890 assertions, and the cached GPU self-test exhaustively matched both output charts for 65,536 token pairs. Eight [Compute Sanitizer runs](../results/integer/sanitizer/commands.json) passed memcheck, racecheck, synccheck and initcheck for both codecs on partial-window, multi-page circulation. The [regression CTest log](../results/integer/final_build_ctest.log) records 12/12 passed; the [Python tool tests](../results/integer/python_tests.log) record 27/27 passed. Those two regression totals include historical paths and are not extra independent quantum-mechanism checks.

All four actual executable binaries passed [PTX and SASS audits](../results/integer/instructions/build_audits.json), with hashes matched against the built files. The explicit native sampler and strictly constant-only HFMA2 materialization exceptions remain in scope. No dynamic-input floating arithmetic/conversion/SFU was found in those audited device instructions. Host reporting, runtime metadata and driver internals are outside that claim.

The [repeated large benchmark](../results/integer/benchmark_large/benchmark.json) uses the same integer law across variants: 32 pages of side 2048, W=65,536 blocks, 256 intervals, batch 32, seed 756, dt 1024, gain 16384, two hops and enabled jitter/mutation. Three rotated sequential trials follow one excluded warmup per configuration. Clocks are not locked. The cached/balanced implementation measures 6.8565 billion BC5 pair updates/s (3.3725x reference) and 5.8638 billion RG8 updates/s (2.5117x). Cached L1-preference measures 7.2165 and 5.9698 billion respectively. The uncached masked shared/texture variants are slower than the direct reference at this workload; cache allocation alone is not an optimization.

Fresh capacity runs allocated 10,619,977,728 BC5 payload bytes (9.890625 GiB; 7,079,985,152 token pairs) and 10,611,589,120 RG8 bytes (9.8828125 GiB; 4,244,635,648 pairs). They initialized their payloads and completed one and two full sweeps respectively, retaining approximately 892 and 900 MiB free. These are exercised large-field allocations with desktop headroom, not a maximum-allocation proof. Profiling the cached BC5 evolve kernel gave 72.38% aggregate L1/TEX hits in balanced mode and 85.47% under L1 preference; balanced DRAM throughput was 12.43% of the profiler's peak metric. These scoped observations do not show DRAM saturation or a permanently locked LUT. See the final summary for profiling setup and the distinction between replay kernel counters and the unprofiled circulation benchmark.
