...
def make_bank(old):
    ...
    for family, successor in [("mutation", False), ("mutation_successor", True)]:
        ...
        signal = add(add(add(sub(V("ai"), V("br")), mul(0.25, add(M, mul(0.5, T)))),mul(.0005,V("dt"))),
        ...
        control = add(V("old_control"), add(mul(0.001, mul(V("dt"),add(M, mul(0.5, T)))), mul(0.002, V("ai"))))
        b.body(family, ["old_body", "target_index", "field_distance", *PAIR, "jitter", "old_control", "mutator_control", "target_field","dt"],
               ...
               requires=[bit_guard(V("jitter")),lt(0,V("dt"))], signature=2)
    ...
    feedback_x=mul(.01,V("feedback_dx"))
    feedback_y=add(mul(.01,V("feedback_y")),mul(.001,sub(1,mul(2,V("feedback_orientation")))))
    kvals = canonical(add(V("ku"), add(mul(0.002, add(sub(w[0], w[2]), d)),feedback_x)),
                      add(V("kv"), mul(sub(1, mul(2, V("ko"))), add(mul(0.002, add(sub(w[1], w[3]), d)),feedback_y))), V("ko"))
    b.body("klein", ["ku", "kv", "ko", *PAIR,"feedback_dx","feedback_y","feedback_orientation", "field_distance"], dict(zip(["ku", "kv", "ko"], kvals)),
           "Recurrent intrinsic Klein chart driven by current field, prior wave and nearest-lift coordinates of the preceding returned flow chart in the oldK frame. Explicit orientation-sensitive feedback; no smooth covariance claim.", requires=[bit_guard(V("ko")),bit_guard(V("feedback_orientation"))])
    ...
    b.body("bst", ["depth", "b0", "b1", "b2", "b3", "neck", "jitter","field_distance"],
           {"index": idx, "routing_phase": add(mul(0.01, d),mul(.001,V("jitter")))}, "Implicit child=2*i+1+bit traversal with explicit neck reversal; supplied jitter affects routing phase, not the address. No modulo or missing-address alias.",
           requires=[eq(V("depth"), op("floor", V("depth"))), sub(1, lt(V("depth"), 0)), sub(1, lt(4, V("depth"))), *[bit_guard(V(k)) for k in ["b0", "b1", "b2", "b3", "neck","jitter"]]])
    # Dependent RK stages; Y-up is a deliberately non-cancelling k4 event.
    omega = add(add(0.2, mul(0.01, d)),mul(.005,sub(mul(2,V("jitter")),1)))
    ...
    b.body("rk4", [*PAIR, "dt", "jitter","field_distance"], {**dict(zip(PAIR, deriv)), "h": heff},
           "Linear complex rotation/damping derivative with explicit supplied-jitter frequency term and field-dependent positive interval; role1 is derivative evaluation.", role=1, requires=[lt(0,V("dt")),bit_guard(V("jitter"))])
    ...
    for i,(key,shape) in enumerate(shapes.items()):
        ...
        guards=[] if key=="apex" else [lt(.12 if key=="T_shape" else 0,radius)]
        b.body(key,[*PAIR,"field_distance"],{"value":shape},f"{key}: {kind}; field controls radius or apex center.",requires=guards)
        ...
        b.body(key,[*PAIR,"routing_phase","field_distance"],{name:add(w[j],mul(scale,[1,-1,0.5,-0.5][(j+i)%4])) for j,name in enumerate(PAIR)},
               f"Typed selected-action adapter for {key}; its primitive value controls a bounded wave displacement.",role=99,signature=90,requires=guards)
    ...
    angle=add(mul(2.39996322972865332,V("delta")),add(mul(0.05,mul(V("dt"),geo)),mul(0.01,d)))
    ...
    b.body("phyllotaxis",["delta",*geom,"dt","field_distance"],dict(zip(PAIR,growth)),"Golden-angle phase-driven candidate pair, coupled to all six current geometric values and the explicit PSI interval; no uniform-sampling claim.",requires=[lt(0,V("dt"))])
...
class Plan:
    ...
    def situated(self,key,role,context,out,dynamic=False):
        ...
        # The initial Klein call sees old K; subsequent calls see the newly
        # evaluated K, retaining the source's direct surface-to-field edge.
        self.helper("nearest_klein_lift",[68,69,219,220,221],223)
...
def mutation_plan(b):
    ...
    p.record(INDEX["mutation"],0,0,2,0,[64,195,183,3,4,5,6,9,76,100,184,8],186)
...
def action_plan(b,cycle):
    ...
    for phase in cycle["phases"]:
        ...
        if phase=="operator_mutation":
            p.helper("placement_canonical",[0,1,2],170)
            p.helper("nearest_klein_lift",[170,171,68,69,70],168)
            p.situated("klein",0,{"ku":68,"kv":69,"ko":70,**pair(64),"feedback_dx":168,"feedback_y":171,"feedback_orientation":172},68)
            ...
            p.situated("bst",0,{"depth":17,"b0":18,"b1":19,"b2":20,"b3":21,"neck":16,"jitter":9},80)
            ...
            p.copy(108,64,4)
            p.situated("rk4",1,{**pair(108),"dt":81,"jitter":9},84)
            p.copy(100,88)
            for derivative_base,previous,fraction in [(88,84,175),(92,88,175),(96,92,176)]:
                ...
                p.situated("rk4",1,{**pair(104),"dt":81,"jitter":9},derivative_base)
            ...
            p.situated("phyllotaxis",0,{"delta":118,**{f"g{i}":112+i for i in range(6)},"dt":81},120)
