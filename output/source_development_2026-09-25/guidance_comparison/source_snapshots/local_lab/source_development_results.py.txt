...
def _diagnostic_checks(diag, training_count, *, evaluated=True):
    # Published guidance can shrink the request set while retaining the old
    # coverage word. Its encoding must remain valid, but it describes the
    # preceding policy until the next native epoch refreshes diagnostics.
    coverage_limit = training_count if evaluated else resident.EXACT_LIMIT
    checks = {
        "accepted_is_bit": diag["accepted"] in (0, 1),
        "admitted_is_bit": diag["admitted"] in (0, 1),
        "coverage_is_bounded_integer": _integer_in(diag["coverage"], 0, coverage_limit),
...
def evaluate_results(checkpoint_bytes, manifest, *, distance_tolerance=2e-5, score_tolerance=2e-5):
    ...
    for lane, instance in enumerate(decoded["instances"]):
        ...
        checks = _diagnostic_checks(diag, len(contract["training_points"]), evaluated=evaluated)
        row = {
            ...
            "diagnostic_coverage_limit": len(contract["training_points"]) if evaluated else resident.EXACT_LIMIT,
