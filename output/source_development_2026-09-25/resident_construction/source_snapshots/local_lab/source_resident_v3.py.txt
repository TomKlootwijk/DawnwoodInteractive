...
def decode_checkpoint(raw):
    ...
    if len(raw) < 40 or raw[:8] != MAGIC:
        raise ValueError("Checkpoint requires DWRD0003 and its eight-word header")
    ...
    uint(state_width, "stateWidth", MAX_STATES, 1)
...
def compile_definition(model, definition):
    ...
    state_names = names(definition["state_names"], "state_names", 1, MAX_STATES)
    ...
    manifest = {
        ...
        "binding_contract": {
            ...
            "continuation": "DWRD0003 checkpoints include the complete static bank and plans plus every instance state and live record word.",
        ...
        "limits": {"records": MAX_RECORDS, "state_floats": MAX_STATES, "functions": MAX_FUNCTIONS,
...
def inspect_checkpoint(path, manifest_path=None):
    ...
    if manifest_path is not None:
        ...
        if not isinstance(manifest, dict) or manifest.get("profile") != PROFILE:
            raise ValueError("Supplied manifest does not describe DWI-RESIDENT-0.3")
