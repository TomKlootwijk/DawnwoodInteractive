"""Compile and inspect a bounded resident-definition numerical component.

The definition explicitly binds selected source records to a finite XIR function
bank and supplies call plans. It does not infer or lower the full source cycle.
"""
...
DEFINITION_KEYS = {
    ...
    "mutation_plan", "action_plan", "instances", "source", "meaning", "status",
}
...
def validate_plan(plan, mutation, metadata, functions, signatures, state_width):
    ...
    for step, words in enumerate(plan):
        ...
        opcode = words[0]
        used = {0: 3, 1: 4, 2: 6, 3: 4, 4: 7, 5: 4, 6: 4, 7: 2, 8: 2, 9: 2, 10: 4}.get(opcode)
        if used is None:
        ...
        if opcode == 0:
            ...
            selector, slot, bank, input_base, output_base, signature = words[1:7]
            ...
            if any(metadata[index][slot + 1] != signature for index in indices):
                raise ValueError(f"{where}: stated call signature does not match the selected immutable slot")
...
def validate_record(words, metadata, functions, where):
    for slot in range(3):
        handle = words[slot]
        if handle >= len(functions):
            raise ValueError(f"{where}.{SLOTS[slot]}: function handle is outside the bank")
        if functions[handle]["signature"] != metadata[slot + 1]:
            raise ValueError(f"{where}.{SLOTS[slot]}: handle has the wrong slot signature")
    uint(words[3], where + ".generation", EXACT_LIMIT)
    for index, word in enumerate(words[4:]):
        finite_word(word, f"{where}.values[{index}]")


def validate_failure_header(header, records, mutation_plan, action_plan, functions, heap, where):
    ...
    if status == 1:
        ...
        def possible(function):
            if not 1 <= ordinal <= function["instruction_count"] or not 1 <= reason <= 4:
                return False
            opcode = heap[function["code_offset"] + 4 * (ordinal - 1)]
            return reason == 3 or (reason == 1 and opcode == 5) or (reason == 2 and opcode == 9) or (reason == 4 and opcode == 17)

        if words[0] == 3:
            valid = possible(functions[words[1]])
        elif words[0] == 4:
            if phase == 1:
                selector = target if words[1] == TARGET else words[1]
                handle = records[selector][SLOTS[words[2]] + "_handle"]
                valid = possible(functions[handle])
            else:
                # Rolled-back action records cannot reveal their discarded
                # candidate handle. Validate against its declared signature.
                valid = any(possible(function) for function in functions if function["signature"] == words[6])
    elif status == 2:
        valid = (post_validation and detail < 3) or (words[0] == 4 and detail >= len(functions))
    elif status == 3:
        valid = phase == 1 and words[0] == 5 and words[1] <= detail < words[1] + words[3] and detail < 4
    elif status == 4:
        valid = (post_validation and detail < 3) or (words[0] == 4 and detail == words[2])
    elif status == 5:
        valid = post_validation and detail == 3
    elif status == 6:
        record_write = phase == 1 and words[0] == 5 and words[1] <= detail < words[1] + words[3]
        state_write = phase == 2 and words[0] == 6 and words[1] <= detail < words[1] + words[3]
        valid = ((post_validation and 4 <= detail < RECORD_WORDS) or record_write or state_write or
                 (words[0] == 7 and detail == words[1]))
    elif status == 7:
        valid = words[0] == 7 and detail == words[1]
    if not valid:
...
def decode_checkpoint(raw):
    ...
    if len(raw) < 40 or raw[:8] != MAGIC:
        raise ValueError("Checkpoint requires DWRD0001 and its eight-word header")
    header = list(struct.unpack_from("<8I", raw, 8))
    count, record_count, state_width, function_count, heap_words, mutation_steps, action_steps, reserved = header
    ...
    if reserved:
        raise ValueError("Checkpoint reserved header word must be zero")
    static_words = checked_size(8 + 4 * record_count + 6 * function_count + heap_words +
    ...
    validate_plan(mutation_plan, True, metadata, functions, signatures, state_width)
    validate_plan(action_plan, False, metadata, functions, signatures, state_width)
    ...
    for lane in range(count):
        ...
        for ordinal in range(record_count):
            ...
            validate_record(words, metadata[ordinal], functions, f"{where}.records[{ordinal}]")
            records.append({
        ...
        validate_failure_header(failure_header, records, mutation_plan, action_plan, functions, heap, where)
        instances.append({
...
def compile_definition(model, definition):
    ir.exact_keys(definition, DEFINITION_KEYS, {
        "profile", "functions", "records", "state_names", "mutation_plan", "action_plan", "instances",
    }, "definition")
    ...
    for ordinal, record in enumerate(records):
        ...
        for slot in SLOTS:
            ...
            if not isinstance(function_name, str) or function_name not in handles:
                raise ValueError(f"{where}.{slot}: expected a declared function name")
            slot_signatures.append(functions[handles[function_name]]["signature"])
        ...
        selected.append({
            ...
            "initial_slot_bindings": {slot: {"function": record[slot], "handle": handles[record[slot]],
                                              "signature": slot_signatures[index]}
    ...
    validate_plan(mutation_plan, True, metadata, functions, signatures, len(state_names))
    validate_plan(action_plan, False, metadata, functions, signatures, len(state_names))
    ...
    def pack_record(record, ordinal, where):
        ...
        for slot in SLOTS:
            name = record[slot]
            if not isinstance(name, str) or name not in handles:
                raise ValueError(f"{where}.{slot}: expected a declared function name")
            result.append(handles[name])
        ...
        validate_record(result, metadata[ordinal], functions, where)
        return result
    ...
    static_words = checked_size(8 + 4 * len(records) + 6 * len(functions) + len(heap) +
    ...
    header = [len(instances), len(records), len(state_names), len(functions), len(heap),
              len(mutation_plan), len(action_plan), 0]
    static = list(header)
    ...
    manifest = {
        ...
        "scope": "selected_resident_definition_mutation_and_action_component",
        "status": "partial_lowering", "complete_application_lowered": False,
        ...
        "selected_records": selected, "functions": function_manifest,
        ...
        "binding_contract": {
            ...
            "continuation": "DWRD0001 checkpoints include the complete static bank and plans plus every instance state and live record word.",
            "arbitrary_expression_synthesis": False,
            "catalogue_growth": False,
            "complete_31_operator_catalogue_claimed": False,
            "cycle_input": "No source cycle file is consumed or implicitly lowered.",
            "not_lowered": ["complete eight-stage source cycle", "automatic lowering of unselected source records",
...
def inspect_checkpoint(path, manifest_path=None):
    ...
    if manifest_path is not None:
        ...
        if not isinstance(manifest, dict) or manifest.get("profile") != PROFILE:
            raise ValueError("Supplied manifest does not describe DWI-RESIDENT-0.1")
        ...
        for instance in result["instances"]:
            ...
            for ordinal, record in enumerate(instance["records"]):
                ...
                record["slot_functions"] = {slot: function_names[record[slot + "_handle"]] for slot in SLOTS}
