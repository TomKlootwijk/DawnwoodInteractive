...
from . import source_development as development_source
...
def _verified_source(decoded, manifest, where):
    ...
    if len(functions) != len(decoded["functions"]) or len(set(function_names)) != len(function_names):
        raise ValueError(f"{where}: named function bank must account for every unique native handle")
    family_names = [item["name"] for item in manifest["families"]]
    if (len(family_names) != len(decoded["families"])
            or len(set(family_names)) != len(family_names)):
        raise ValueError(f"{where}: named family bank must account for every unique native handle")
    numerical = []
    ...
    # A self-consistent edited bank is insufficient: this edition protects its
    # gates, objective and interpreter through the actual authored declaration.
    development_source.validate_spec(authored)
    if not _json_equal(source.get("development"), development):
        raise ValueError(f"{where}: duplicated development declarations disagree")
    ...
    # Reconstruct only named definitions/layout from already verified manifest
    # identities. Neither native tapes nor their call-site metadata are inputs
    # to graph lowering. Live values do not determine the immutable call graph.
    definition = {
        "source": deepcopy(source), "state_names": deepcopy(manifest["state_names"]),
        "record_value_names": deepcopy(manifest["record_value_names"]),
        "functions": {item["name"]: {"signature": item["signature"],
            "binding": deepcopy(item["binding"])} for item in functions},
        "families": {item["name"]: {"interface": item["interface"],
            "methods": deepcopy(item["method_functions"])} for item in manifest["families"]},
        "records": [{"key": item["key"], **{slot: item["initial_slot_bindings"][slot]["definition"]
            for slot in ("body", "field", "placement")}} for item in manifest["selected_records"]],
    }
    cycle = source.get("cycle_definition")
    if not isinstance(cycle, dict):
        raise ValueError(f"{where}: actual source cycle is required for publication")
    structural = development_fidelity.audit_development_graph(definition, cycle, graph["graph"])
    # Graph helpers were appended by the original lowering. Remove that exact
    # suffix, then let the generic compiler reconstruct it and both plans.
    helpers = list(graph["graph"].get("functions", {}))
    if helpers and function_names[-len(helpers):] != helpers:
        raise ValueError(f"{where}: graph helper handles are not the declared final bank suffix")
    for name in helpers:
        del definition["functions"][name]
    graph_compiler.apply_graph(definition, cycle, graph["graph"], resident_backend=resident)
    for phase in ("mutation", "action"):
        if definition[phase + "_plan"] != decoded[phase + "_plan"]:
            raise ValueError(f"{where}: actual native {phase} plan differs from the checked source graph")
    if list(definition["functions"]) != function_names or any(
            definition["functions"][item["name"]]["signature"] != item["signature"]
            or not _json_equal(definition["functions"][item["name"]]["binding"], item["binding"])
            for item in functions):
        raise ValueError(f"{where}: source graph lowering changes the declared function identities")
    ...
    return {"functions": numerical, "families": family_identities,
            ...
            "source_validation": {"edition_declaration_validated": True,
                "protected_bindings_match_actual_native_functions": True,
                "structural_audit_profile": structural["profile"],
                "structural_audit_sha256": _json_sha(structural),
                "both_native_plans_relowered_from_named_graph": True}}
...
def migrate_guidance(parent_checkpoint_bytes, parent_manifest, new_program_bytes, new_manifest, *, query=None):
    ...
    Protected source validation, pure-binding recompile, structural source-edge
    audit and fresh graph lowering cross-check the actual executable plans.
    ...
    publication = {
        ...
        "source_validation": {"parent": old_source["source_validation"], "new": new_source["source_validation"]},
        ...
        "limitations": [
            ...
            "Protected source declarations, native function words, structural source edges and both relowered native plans are checked. This uses the current compiler/audit implementation, not an independent formal proof of every mathematical or source relationship.",
